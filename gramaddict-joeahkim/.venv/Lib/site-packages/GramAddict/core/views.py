...
class PostsViewList:
    ...
    def swipe_to_fit_posts(self, swipe: SwipeTo):
        ...
        if swipe == SwipeTo.HALF_PHOTO:
            media_container_obj = self.device.find(
                resourceIdMatches=containers_content
            )
            if not media_container_obj.exists():
                logger.warning("Media container not found, using generic scroll.")
                ...
                return
            zoomable_view_container = media_container_obj.get_bounds()["bottom"]
            ...
            for _ in range(3):
                if not gap_view_obj.exists():
                    ...
                    if not media.exists():
                        logger.warning("Media container not found in NEXT_POST swipe, breaking.")
                        break
                    if (
            ...
            if obj1 is None:
                if gap_view_obj.exists():
                    ...
                    logger.warning("Gap view not found in NEXT_POST swipe, using generic scroll.")
                    displayHeight = self.device.get_info()["displayHeight"]
                    self.device.swipe_points(
                        displayWidth / 2,
                        displayHeight * 0.6,
            ...
            if not containers_content.exists():
                logger.warning("Media container not found after NEXT_POST loop, using generic scroll.")
                displayHeight = self.device.get_info()["displayHeight"]
                self.device.swipe_points(
                    displayWidth / 2,
                    displayHeight * 0.6,
    ...
    def _like_in_post_view(
        ...
        if not already_watched:
            media_type, obj_count = post_view_list.detect_media_type(content_desc)
            if media_type == MediaType.CAROUSEL and obj_count == 0:
                logger.warning("Broken carousel with 0 items detected, skipping like.")
                return
    ...
    def _check_if_liked(self, retries=3):
        ...
        if bnt_like_obj.exists():
            ...
            if retries <= 0:
                logger.warning("Like button not found after multiple scrolls, assuming not liked.")
                return False
            UniversalActions(self.device)._swipe_points(
